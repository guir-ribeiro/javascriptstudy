...
if(isChild || isOlder){
    priceMessage = 'a entrada é grátis!';
}else{
...
for(let i = 0; i < randomNumbers.length; i++){
    ...
    if(par){
        parNumbers.push(item);
    }
    else{
        imparNumbers.push(item);
    }
}
// console.log(`Numeros ímpares: ${imparNumbers.join(', ').replace(', 3', ' e 3')}. Números pares: ${parNumbers.join(', ').replace(', 5', ' e 5')}.`)